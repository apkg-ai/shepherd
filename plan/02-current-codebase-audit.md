...
| core/shepherd-core/src/dag.rs | Pure cycle and single-parent checks | Remove in 000; reconsider the algorithm from Git history in 005 under new ownership tests |
| core/shepherd-core/src/lease.rs | TTL mechanics | Remove in 000; reconsider bounded time calculations from Git history in 009 under transaction tests |
...
| core/shepherd-server/src/lib.rs | Thin generated-trait implementation and SSE | Reset to health/static shell; rebuild resource handlers and durable replay in 015 |
...
| ui/src/screens/graph | React Flow selection/side panels | Remove product screens in 000; reuse generic canvas ideas from history in 018 if still valid |
...
| ui/src/api/client.ts, lib/events.ts | Central fetch/error handling; refetch on SSE reconnect | Reset to scaffold transport; build authenticated client in 016 and durable events in 020 |
| ui/src/components and styles | Reusable accessible primitives and tokens | Preserve; extend badges for phase and eligibility |

The reset preserves engineering capability rather than compiled MVP behavior: workspace/crate boundaries, package and toolchain pins, CI/security/dependency jobs, contract generation, coverage reporting, Hurl/Playwright/Vitest infrastructure, the server/UI boot path, generic accessible components and design tokens. Product modules and semantic fixtures are deleted. Later steps may recover a small algorithm from Git history only after testing it against the stable-v1 contracts.

Step 000 owns the exact keep/remove inventory and must leave all quality categories green. It replaces obsolete behavioral tests with scaffold checks instead of disabling jobs or retaining contradictory product code.
