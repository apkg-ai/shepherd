...
getEvents, exportProject and importProject remain in OpenAPI but are excluded from generated Axum handler whitelist. Implement them as handwritten Axum Body/stream routes with the same authentication/errors/schema conformance. Normal generated validation has a 2 MiB body cap; import must use the separate authenticated staged streaming parser. Enforce the import's 30-second idle-progress timeout while receiving body chunks and validating records, and delete the staging database on timeout or disconnect. Shared client/CLI export/import copy byte streams rather than deserialize a whole ExportDocument into memory. Typed record models still come from the same contract. Other 67 operations use generated handlers.
