...
- `.github/workflows/{quality-gates,dependency-scan,sast}.yaml` (quality-gates minus the two removed jobs and the AsyncAPI lint step; scan/SAST since updated in review follow-ups — osv-scanner 2.5.1→2.6.0, semgrep 1.176.1→1.177.0 with the scan widened to eleven packs excluding `plan/` and `*.test.tsx`, plus a sha256-pinned shellcheck gate)
- `scripts/{regen-generated.sh,coverage-report.ts,smoke.sh,hurl-e2e.sh,playwright-e2e.sh}` (the last three lose temp-DB/`--db` plumbing; hurl runner loses the SSE section; coverage-report converted from `.mjs` to TypeScript in a review follow-up); `tests/hurl/01-health-and-spec.hurl`
...
- `node scripts/coverage-report.ts --check` with all four lcov suites — thresholds met: core unit 100%, integration 100%, total 100%; ui unit 97.1%, e2e 69.7%, total 94.5%.
