# 022 — Complete CLI adapter

Status: not started. Requirements: API-01.

## Objective and prerequisites

Deliver complete cli adapter. Required completed steps: [021](021-shared-rest-client.md)
...
- `core/Cargo.toml`
- `core/shepherd-cli/Cargo.toml`
- `core/shepherd-cli/src/{main,args,output}.rs`
- `core/shepherd-cli/tests/cli.rs`
...
2. Add shepherd-cli crate/bin shepherd and add `shepherd-cli` to the explicit workspace members in `core/Cargo.toml`. Implement api <operationId> with common parameters plus ergonomic alias table from CLI doc. Support input/output files and JSON mode, owner/agent token file, lease file and explicit idempotency. Special streaming command handles SSE; maintenance commands remain server binary.
...
Every supported catalog operation dispatches through shared client; no hidden database access. Exit codes match error table. stdout JSON parses, diagnostics only stderr, token issuance/lease file permissions verified. Round-trip planning/execution scenario works against real daemon.
