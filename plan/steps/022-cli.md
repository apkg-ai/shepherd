...
2. Add shepherd-cli crate/bin shepherd. Implement api <operationId> with common parameters plus ergonomic alias table from CLI doc. Support input/output files and JSON mode, owner/agent token file, lease file and explicit idempotency. Special streaming command handles SSE; maintenance commands remain server binary.
