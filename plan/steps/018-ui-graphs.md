# 018 — Navigable scoped dependency graphs

Status: not started. Requirements: UI-01 DEP-01.

## Objective and prerequisites

Deliver navigable scoped dependency graphs. Required completed steps: [017](017-ui-task-details.md)
...
- `ui/src/screens/graph/{GraphScreen,GraphSidePanel,GraphActions}.tsx (retain existing .ts where applicable)`
- `ui/src/lib/graphLayout.ts`
- `ui/src/lib/taskTree.ts (remove after imports migrated)`
...
2. Replace taskTree/decomposition adapter with getGoalGraph/getEpicGraph. Reuse React Flow/Dagre canvas, focus and neighbor highlighting. Implement create/remove dependency dialogs, arrows prerequisite→dependent, selected side panel and drill-down. Cache viewport per route and relayout only on topology change.
...
Branch/join/disconnected graphs render; cross-scope candidates absent and server rejection displayed. Double-click/Enter drills down, breadcrumb returns. Claim heartbeat does not reset viewport. Cycles/active-work failures preserve selection. Large graph limit shows list fallback.
