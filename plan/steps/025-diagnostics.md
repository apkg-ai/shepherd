# 025 — Integrity diagnostics and graceful shutdown

Status: not started. Requirements: OPS-01.

## Objective and prerequisites

Deliver integrity diagnostics and graceful shutdown. Required completed steps: [015](015-rest-integration.md), [014](014-portability.md)
...
- `core/shepherd-server/src/{diagnostics,logging,main}.rs`
- `core/shepherd-core/src/storage/connect.rs`
...
2. Add structured tracing subscriber/log redaction, request IDs, schema/integrity checks, health fail behavior, doctor command and signal shutdown. Exercise storage_busy response and sweeper failures. Keep diagnostic-only mode on corruption/key mismatch; no silent repair.
...
Kill process at report failpoints then restart: DB valid with all-or-nothing result. Graceful stop releases lock and finishes/rolls back transactions. Logs contain request IDs and no secrets/content. doctor reports actionable resource IDs.
